...
### <관리자_고객관리>
1. 고객 정보 관리자(“managing Customers”)는 개인 정보, 연락처 정보, 재무 정보, 고용 이력, 신용 점수 등 모든 고객 정보를 보고 관리할 수 있다. 
2. 이후에 신용점수, 소득, 직업 분류를 고려하여 대출 승인 처리 유무를 가릴 수 있다.
3. 검색 기능을 이용하여 효율적으로 고객을 찾을 수 있다.

<br>

### <관리자_대출관리>
1. 고객에게 제공하는 대출 상품들의 정보를 조회, 추가 및 삭제 할 수 있다.
2. 해당 업무는 상품 관리자(“managing Products”) 권한을 갖고 있는 관리자가 수행할 수 있다.
3. 은행 관리자는 대출관리 시스템을 통해 전세 대출, 담보 대출 등과 같은 대출 상품을 등록 및 삭제할 수 있다. 
4. 상품 종류에 따라 전체, 전세, 월세, 담보의 상품만 선택하여 조회할 수 있다.
5. 각 대출 상품은 고유번호, 상품 종류, 이름 이율, 연체이율, 최대한도, 기간, 최소신용점수 등의 속성을 가지고 있다. 이러한 속성들은 대출 상품의 특성을 정의하고, 고객이 대출 상품을 선택하는 데 필요한 정보를 제공한다.
    - 대출 상품 정보
        - 상품명: 대출 상품명
        - (연체)이자율: 대출 상품의 연간 이자율. 이는 고정 금리 또는 시장 상황에 따라 변경되는 변동금리일 수 있음.
        - 대출 기간: 대출 상환 기간. 이것은 몇 달 또는 몇 년으로 표현될 수 있습니다.
        - 대출 유형: 대출 유형(전월세 대출, 담보 대출).
        - 대출 한도: 본 대출 상품으로 빌릴 수 있는 최대 금액입니다.
        - 자격 기준: 차용인이 대출 상품에 대한 자격을 갖추기 위해 충족해야 하는 기준. (최소 신용 점수, 최소 소득)

6. 신용점수, 직업, 소득 등과 같은 고객의 정보는 대출 상품을 이용할 수 있는 고객의 범위를 결정하는 요소이다.
7. 대출 상품에는 최소 신용 등급이 설정되어 있다. 신용 점수로 구분하여 0점부터 1000점까지 존재한다.
8. 직장 분류로는 professional(전문직), public servant(공직), freelancer(개인 사업자), daily worker(일용직)로 구성되어 있다. 각 분류에 따라 점수는 100, 70, 50, 20점으로 다르게 분배된다.
9. 소득은 연봉을 의미한다.
10. 대출 상품을 등록할 때는 상품명, 상품 구분, 상품 설명, 이율 등 고객들에게 제공할 정보들을 모두 입력해야 등록된다.
11. 대출 상품 삭제는 해당 상품 정보 가장 오른쪽에 위치해있는 삭제 버튼을 누르면 삭제되며, 만약 해당 권한이 없는 관리자가 누르게 되면 권한이 없다는 메시지가 나오게 된다.

<br>

### <관리자_상환관리>
1. 고객의 대출 상환 추적 및 관리를 용이하게 한다.
2. 고객명, 상품 구분, 상품명, 대출 신청일, 마감기한, 잔액, 지난 날짜, 상환 상태 등의 고객들의 대출 상품 상환 내역을 조회할 수 있다.
3. 상품 종류에 따라 전체, 전세, 월세, 담보의 상품들을 선택하여 조회할 수 있다.
4. 상환 상태에 따라 전체, 상환중, 연체, 상환 완료 상품들을 선택하여 조회할 수 있다.
5. 상환 일정을 설정하고 이자를 계산하는 옵션을 제공한다.
6. 새로운 고객 상호 작용에 대해 관리자에게 알리는 알림 시스템을 구현한다.
7. 대출 상품 관리자(“managing Products”) 권한을 갖고 있는 관리자가 지불 금액 및 날짜를 ​​포함한 상환 정보를 기록하고 업데이트하도록 허용한다.
8. 모든 대출 상품들은 만기상환 이기 때문에 고객이 금액을 모두 상환을 했다면 관리자가 잔액 차감을 해줄 수 있다.
9. 상환 기한이 지나 연체가 되었다면 관리자는 연체 관리 버튼을 눌러 이자율이 업데이트 된다. 이자율은 매년 연체이자율에 맞춰 계산되어 갚아야 하는 잔액에 들어가게 된다. 
10. 연체된 지불에 대한 경고 및 알림을 고객에게 제공한다.
...
1. 페이지 상단에 GNB가 제공된다.
    - 로고 클릭 시 사용자 홈으로 이동한다.
    - 조회, 이체, 금융상품 등 메뉴 클릭 시 해당 페이지로 이동한다. (대출 상품 버튼만 작동)
    - 로그아웃 버튼 클릭 시 로그아웃 되며 로그인페이지로 이동한다.
2. 광고 및 추천상품 표시영역에서 사용자 개인의 성향 및 서비스 이용 로그 분석을 통해 적절한 상품에 대한 광고 및 추천
    - 광고 및 추천상품이 좌->우 방향 슬라이드 효과와 함께 노출된다.
    - 클릭 시 해당 상품상세페이지로 이동한다.
3. 로그인한 계정의 요약 정보를 한 눈에 쉽게 볼 수 있는 회원정보영역이 제공된다.
    - 고객의 회원정보가 요약되어 노출된다. 
    - 고객이 가입한 대출 상품의 대출 금액, 이자율, 상환 일정 및 대출 상태와 같은 세부 정보가 요약되어 노출된다.
4. 추천 서비스
